...
var roomConfig = {
    roomName: "Futsal V4 Qatar Kaptanlı YS",
    maxPlayers: 12,
    public: true,
    noPlayer: true,
...
var config = {
    ...
    adminAuth: "zAPJ44NRbKRzzNaaTN15DVN-OL_Q8mBKRWoKzGLRxwk", // AUTHUNU EKLE TADINI ÇIKAR
    maxPlayersPerTeam: 4,
...
function choosePlayer() {
    ...
    room.sendAnnouncement("ᴛɪᴘꜱ: ᴇɴᴛᴇʀ ᴘʟᴀʏᴇʀ ɴᴜᴍʙᴇʀ ᴏʀ ᴛʏᴘᴇ 'ʀᴀɴᴅᴏᴍ'", captain.id, 0x69ADFE, "normal");
    getSpecList(captain);
    
    timeOutCap = setTimeout(function(cap) {
        room.sendAnnouncement("Kaptan " + cap.name + " oyuncu seçiyor...", null, 0x55bae2, "small");
        room.sendAnnouncement("⚠️ Hızlı ol! Sadece " + Number.parseInt(chooseTime / 2) + " saniye kaldı!", cap.id, 0xf2a000, "normal");
...
room.onGameStop = function(byPlayer) {
    ...
    if (byPlayer == null) {
        ...
        if (scores.red > scores.blue) {
            winnerTeam = 1;
            room.sendAnnouncement("🏆 KIRMIZI TAKIM KAZANDI!", null, 0xFF3B3B, "bold");
            
            updateQueue();
            if (specQueue.length === 0) {
                room.sendAnnouncement("📋 Spec'te oyuncu yok, oyun devam ediyor...", null, 0xFFC107, "normal");
                setTimeout(function() {
                    room.startGame();
                }, 3000);
            } else {
                room.sendAnnouncement("🔄 Mavi takım spec'e gönderiliyor...", null, 0xFF9800, "normal");
                
                setTimeout(function() {
                    var bluePlayers = getTeam(2);
                    bluePlayers.forEach(function(p) {
            ...
            winnerTeam = 2;
            room.sendAnnouncement("🏆 MAVİ TAKIM KAZANDI!", null, 0x0080FF, "bold");
            ...
            if (specQueue.length === 0) {
                ...
                room.sendAnnouncement("🔄 Kırmızı takım spec'e gönderiliyor...", null, 0xFF9800, "normal");
                
                setTimeout(function() {
                    var redPlayers = getTeam(1);
                    redPlayers.forEach(function(p) {
                        room.setPlayerTeam(p.id, 0);
                    });
            ...
            room.sendAnnouncement("🤝 Oyun berabere bitti!", null, 0xFFC107, "normal");
            ...
            if (specQueue.length === 0) {
                ...
                room.sendAnnouncement("🎲 Beraberlik! Rastgele " + loserName + " takım spec'e gönderiliyor...", null, 0xFF9800, "normal");
                
                setTimeout(function() {
                    var loserPlayers = getTeam(loserTeam);
                    loserPlayers.forEach(function(p) {
                        room.setPlayerTeam(p.id, 0);
                    });
