...
// =============================================================================
// MAP BURAYA GELECEK - SEN EKLEYECEKSİN
// =============================================================================
...
function pickBottom(callerTeam, callerName) { 
    ...
    var result = specQueue.length > 0 ? pickPlayer(specQueue.length, callerTeam, callerName) : false;
    return result;
}
...
function choosePlayer() {
    ...
    // Sadece seçim yapacak takıma komutları göster
    var choosingPlayers = getTeam(choosingTeam);
    choosingPlayers.forEach(function(p) {
        showQueue(p.id);
        msg("💡 Komutlar: 1, 2, 3... | random | top | bottom", colors.success, p.id);
    });
}
// =============================================================================
// GOL EFEKTLERİ 
...
if (/^\d+$/.test(message)) {
    ...
    pickPlayer(num, p.team, p.name);  // result değişkenini kaldır
    return false;
}

if (msgLower === "random" || msgLower === "rand") {
    pickRandom(p.team, p.name);  // result değişkenini kaldır
    return false;
}

if (msgLower === "top" || msgLower === "auto") {
    pickTop(p.team, p.name);  // result değişkenini kaldır
    return false;
}

if (msgLower === "bottom" || msgLower === "bot") {
    pickBottom(p.team, p.name);  // result değişkenini kaldır
    return false;
}
        } else {
...
room.onGameStop = function(byPlayer) {
    ...
    // Oyun bitti - kazanan takımı belirle
    if (byPlayer == null) { // Manuel değil, oyun bitti
        var scores = room.getScores();
        
        if (scores.red > scores.blue) {
            winnerTeam = 1; // Kırmızı kazandı
            msg("🏆 KIRMIZI TAKIM KAZANDI!", colors.red);
            ...
            if (specQueue.length === 0) {
                ...
                msg("🔄 Mavi takım spec'e gönderiliyor...", colors.warning);
            ...
            winnerTeam = 2; // Mavi kazandı
            msg("🏆 MAVİ TAKIM KAZANDI!", colors.blue);
    ...
    resetDiscs(); // Efekt disklerini temizle
};
