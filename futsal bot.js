...
function choosePlayer() {
    ...
    // Takımlar tam oldu mu kontrol et
    ...
    // Hangi takım seçim yapıyor bildir
    var choosingTeam = winnerTeam === 1 ? 2 : 1; // Kaybeden takım
...
function pickPlayer(position, callerTeam, callerName) {
    ...
    // Sadece kaybeden takım seçebilir
...
function checkTeamsBalance() {
    if (getTeam(1).length === config.maxPlayersPerTeam && getTeam(2).length === config.maxPlayersPerTeam) {
        ...
        msg("✅ Takımlar tam! (4v4) Oyun başlatılıyor...", colors.success);
        setTimeout(function() { room.startGame(); }, 2000);
    }
}
...
function startAutoSelection() {
    updateQueue();
    if (specQueue.length === 0) { msg("⚠️ Spec'te oyuncu yok!", colors.warning); return; }
    selectionActive = true;
    var selectingTeam = winnerTeam === 1 ? 2 : 1;
    var teamColor = selectingTeam === 1 ? colors.red : colors.blue;
    msg("═══════════════════════════════════", colors.bot);
    msg((selectingTeam === 1 ? "🔴 KIRMIZI" : "🔵 MAVİ") + " TAKIM OYUNCU SEÇİYOR", teamColor);
    msg("═══════════════════════════════════", colors.bot);
    showQueue();
    msg("💡 Seçim: 1, 2, 3... | random | top | bottom", colors.success);
}
...
room.onPlayerChat = function(player, message) {
    ...
    // SEÇİM MODU KOMUTLARI - KAYBEDEN TAKIMDAKİ HERKES SEÇEBİLİR
    if (chooseMode && p.team !== 0) {
        var choosingTeam = winnerTeam === 1 ? 2 : 1; // Kaybeden takım
        
        // Eğer oyuncu kaybeden takımdaysa seçim yapabilir
...
room.onPlayerTeamChange = function(changedPlayer, byPlayer) {
    setTimeout(function() { updateQueue(); if (selectionActive) setTimeout(checkTeamsBalance, 200); }, 100);
};
