...
// Gelişmiş Seçim Sistemi + Otomatik Dengeleme + Gol Efektleri
// 📜 Lisans: MIT - İstediğiniz gibi kullanabilir, değiştirebilirsiniz!
// =============================================================================

var roomConfig = {
    roomName: "Futsal V4 YS KAPTANLI",
    maxPlayers: 12,
    public: true,
    noPlayer: true,
...
var config = {
    ...
    maxPlayersPerTeam: 4,
    chooseTime: 30, // Seçim için verilen süre (saniye)
...
function updateTeams() {
    ...
    teamR = players.filter(function(p) { return p.team === 1; });
    teamB = players.filter(function(p) { return p.team === 2; });
    teamS = players.filter(function(p) { return p.team === 0; });
}
...
function updateQueue() {
    updateTeams();
    specQueue = teamS.map(function(p, i) {
...
function getSpecList(player) {
    ...
    if (specQueue.length === 0) {
        msg("📋 Spec listesi boş.", colors.bot, player.id);
        return;
    }
    
    var specList = "────────────────── ᴏʏᴜɴᴄᴜ ɴᴜᴍᴀʀᴀsıɴı ʏᴀᴢıɴ ──────────────────\n";
    specList += "     [0] Random | ";

    for (var i = 0; i < 4 && i < specQueue.length; i++) {
        specList += "[" + (i + 1) + "] " + specQueue[i].name + " | ";
    }
    specList = specList.slice(0, -3);
    specList += "\n     ";

    for (var i = 4; i < specQueue.length; i += 5) {
        for (var j = 0; j < 5 && (i + j) < specQueue.length; j++) {
            specList += "[" + (i + j + 1) + "] " + specQueue[i + j].name + " | ";
        }
        specList = specList.slice(0, -3);
        if (i + 5 < specQueue.length) {
            specList += "\n     ";
        }
    }
...
function pickPlayer(position, callerTeam, callerName, callerId) {
    if (!inChooseMode) {
    ...
    var captain = getTeam(choosingTeam)[0];
    
    if (!captain || callerId !== captain.id) {
        msg("⚠️ Sadece kaptan (" + (captain ? captain.name : "?") + ") oyuncu seçebilir!", colors.warning, callerId);
        return false;
    }
    
    if (callerTeam !== choosingTeam) {
        msg("⚠️ Sadece kaybeden takım oyuncu seçebilir!", colors.warning, callerId);
        return false;
    }
    ...
    if (position < 1 || position > specQueue.length) {
        msg("⚠️ Geçersiz sıra! (1-" + specQueue.length + " arası seçebilirsiniz)", colors.warning, callerId);
        return false;
    }
    
    if (!canTeamPick(callerTeam)) {
        msg("⚠️ Takımınız zaten dolu! (" + getTeam(callerTeam).length + "/" + config.maxPlayersPerTeam + ")", colors.warning, callerId);
        return false;
    }
    ...
    msg("✅ " + selected.name + " → " + teamName + " takıma alındı! (Kaptan: " + callerName + ")", teamColor);
...
function pickRandom(callerTeam, callerName, callerId) {
    if (!inChooseMode) return false;
    
    var choosingTeam = winnerTeam === 1 ? 2 : 1;
    var captain = getTeam(choosingTeam)[0];
    
    if (!captain || callerId !== captain.id) {
        msg("⚠️ Sadece kaptan oyuncu seçebilir!", colors.warning, callerId);
        return false;
    }
    ...
    msg("🎲 " + selected.name + " → Rastgele " + teamName + " takıma alındı! (Kaptan: " + callerName + ")", teamColor);
...
function pickTop(callerTeam, callerName, callerId) {
    if (!inChooseMode) return false;
    ...
    return pickPlayer(1, callerTeam, callerName, callerId);
}

function pickBottom(callerTeam, callerName, callerId) {
    if (!inChooseMode) return false;
    ...
    updateQueue(); 
    return specQueue.length > 0 ? pickPlayer(specQueue.length, callerTeam, callerName, callerId) : false;
}
...
function choosePlayer() {
    ...
    if (!captain) {
        msg("⚠️ Kaptan bulunamadı!", colors.warning);
        return;
    }
    ...
    msg("👑 Kaptan: " + captain.name, teamColor);
    msg("Takım Durumu: 🔴 " + redCount + "/4 | 🔵 " + blueCount + "/4", colors.bot);
    msg("═══════════════════════════════════", colors.bot);
    
    room.sendAnnouncement("ᴛɪᴘꜱ: ᴏʏᴜɴᴄᴜ ɴᴜᴍᴀʀᴀꜱı ɢɪʀɪɴ ᴠᴇʏᴀ 'random' ᴠᴇʏᴀ '0' ʏᴀᴢıɴ", captain.id, 0x69ADFE, "normal");
    getSpecList(captain);
    
    timeOutCap = setTimeout(function(cap) {
        room.sendAnnouncement("Kaptan " + cap.name + " oyuncu seçiyor...", null, 0x55bae2, "small");
        room.sendAnnouncement("⚠️ Hızlı ol! Sadece " + Number.parseInt(config.chooseTime / 2) + " saniye kaldı!", cap.id, 0xf2a000, "normal");
        
        timeOutCap = setTimeout(function(cap2) {
            room.kickPlayer(cap2.id, "AFK - Seçim yapmadı", false);
        }, config.chooseTime * 500, cap);
        
    }, config.chooseTime * 1000, captain);
}
...
function updateRoleOnPlayerOut() {
    ...
    if (inChooseMode) {
        ...
        if (capLeft) {
            ...
            var captain = (teamR.length <= teamB.length && teamR.length > 0) ? teamR[0] : teamB[0];
            if (captain) {
                getSpecList(captain);
            }
        }
...
room.onPlayerJoin = function(player) {
    if (player.auth === config.masterAuth) {
        ...
        msg("🔐 Master yetkisi verildi!", colors.success, player.id);
    }
    ...
    msg("💡 Komutlar: !github", colors.success, player.id);
...
room.onPlayerLeave = function(player) {
    capLeft = inChooseMode && (
        (teamR.length > 0 && player.id === teamR[0].id) || 
        (teamB.length > 0 && player.id === teamB[0].id)
    );
...
room.onPlayerChat = function(player, message) {
    ...
    // Spec sırası göster
    if (msgLower === "q" || msgLower === "queue" || msgLower === "sira" || msgLower === "sıra") {
        showQueue(p.id);
        return false;
    }
    
    // Seçim modu komutları
    if (inChooseMode && p.team !== 0) {
        var choosingTeam = winnerTeam === 1 ? 2 : 1;
        
        if (p.team === choosingTeam) {
            // 0 veya Random seçimi
            if (message === "0" || msgLower === "random" || msgLower === "rand") {
                pickRandom(p.team, p.name, p.id);
                return false;
            }
            ...
            if (/^\d+$/.test(message)) {
                ...
                pickPlayer(num, p.team, p.name, p.id);
                return false;
            }
            
            // Top seçim
            if (msgLower === "top" || msgLower === "auto") {
                pickTop(p.team, p.name, p.id);
                return false;
            }
            
            // Bottom seçim
            if (msgLower === "bottom" || msgLower === "bot") {
                pickBottom(p.team, p.name, p.id);
                return false;
            }
        } else {
...
room.onGameStop = function(byPlayer) {
    ...
    if (byPlayer == null) {
        ...
        if (scores && scores.red !== scores.blue) {
            ...
            var winnerName = winnerTeam === 1 ? "🔴 KIRMIZI" : "🔵 MAVİ";
            var winnerColor = winnerTeam === 1 ? colors.red : colors.blue;
            
            msg("", null);
            msg("═══════════════════════════════════", colors.success);
            msg(winnerName + " TAKIM KAZANDI!", winnerColor);
            msg("═══════════════════════════════════", colors.success);
            msg("", null);
            ...
            if (specQueue.length === 0) {
                msg("📋 Spec'te oyuncu yok, oyun devam ediyor...", colors.warning);
                setTimeout(function() {
                    room.startGame();
                }, 3000);
            } else {
            ...
            if (specQueue.length === 0) {
                ...
                msg("🎲 Beraberlik! Rastgele " + loserName + " takım spec'e gönderiliyor...", colors.warning);
                
                setTimeout(function() {
                    moveLosingTeamToSpec(loserTeam);
...
room.onTeamGoal = function(team) {
    ...
    if (lastPlayersTouched[0] != null && lastPlayersTouched[0].team === team) {
        ...
        if (lastPlayersTouched[1] != null && lastPlayersTouched[1].team === team) {
            ...
            msg("⚽ GOL! " + scorer.name + " | " + scores.name + " | " + scores.red + " - " + scores.blue, teamColor);
        }
...
setTimeout(function() {
    ...
    msg("🎮 Bot aktif! Gol efektleri ve seçim sistemi hazır!", colors.bot);
    msg("👑 Sadece KAPTANLAR oyuncu seçebilir!", colors.warning);
}, 1000);
